...
<v-app>
  <f7-page>
    <f7-navbar title="Cost Approximater"  sliding></f7-navbar>
    <f7-block inner>
...
<article>                      
      ...
      <p class="location-name">Hello<input id="name-field" label="Value" :size="inputNameSize" 
        v-model="quotename" placeholder="Client Name" class="sel-one"></input>,
      
       <p class="location-name">  Where will the job start? :
      <input id="pickup-location" class="sel-one" v-model="pickupLocation" :size="inputStartLocationSize" placeholder="pickup location">.
      </p>
       <p class="location-name">
      Next, the destination
      <input id="start" :size="inputFinalLocationSize"  v-model="finalLocation"  placeholder="Final unload location, We Assume Blank means that it's an on-premise move.">. </p>
 ...
 <button v-on:click="resetForm()" class="btn btn-primary btn-sm">Reset</button>-->
    ...
    <p class="location-name">
      When
      <input id="move-date" type="date" v-model="submitDate">
       do you need to to use our professional movers ?
    </p>
     <div class="text-xs-center">
          <button id="calculate-quote" round color="primary"   v-on:click="onCalculateQuote" dark>Calculate Quote Test</button>

 <!--   <v-btn id="calculate-quote" round color="primary"   v-on:click="onCalculateQuote" dark>Calculate Quote</v-btn> -->
  </div>
...
<v-dialog v-model="dialog" max-width="350">
  <QuoteEstimator  v-if="local" :duration="duration" :pickupLocation="pickupLocation" 
      :minHours="3"  
        :finalLocation="finalLocation" :distance="distance"  :hourlyRate="hourlyRate" :baseDayRate="hourlyRate" :driveTime="driveTime" 
        :submitDate="submitDate" :customerName="quotename" :errorMsg="errorMsg"/>
      
        <ShortHaulEstimator v-if="shortHaul" :duration="duration" :pickupLocation="pickupLocation" 
      :minHours="3"  :minPoundage="3000"
        :finalLocation="finalLocation" :distance="distance"  :hourlyRate="hourlyRate" :baseDayRate="hourlyRate" :driveTime="driveTime" 
        :submitDate="submitDate" :customerName="quotename" :errorMsg="errorMsg"/>

         <IntraStateEstimator v-if="intraState" :duration="duration" :pickupLocation="pickupLocation" 
      :minHours="3"  :minPoundage="2100"
        :finalLocation="finalLocation" :distance="distance"  :hourlyRate="hourlyRate" :baseDayRate="hourlyRate" :driveTime="driveTime" 
        :submitDate="submitDate" :customerName="quotename" :errorMsg="errorMsg"/>
    </v-dialog>
